 ...
 *   2. Inserts a demo organization (admin as owner) + default site settings into local D1.
 *
...
const BASE_URL = env.SEED_BASE_URL || "http://localhost:3030";
...
function d1(sql: string) {
  ...
  const file = join(
    tmpdir(),
    `seed-${Date.now()}-${Math.random().toString(36).slice(2)}.sql`
  );
...
function seedDemoRows() {
  ...
  // Better Auth's `organization` table is the only org table. The admin becomes the
  // demo org's owner so it can open /${orgSlug}/app straight away.
  d1(
    `INSERT OR IGNORE INTO organization (id, slug, name, status, created_at)
     VALUES ('${orgId}', '${orgSlug}', 'Demo Organization', 'active', ${Date.now()});
     INSERT OR IGNORE INTO member (id, organization_id, user_id, role, created_at)
     SELECT 'member_demo_owner', '${orgId}', id, 'owner', ${Date.now()} FROM user WHERE email = '${adminEmail}';`
  );
  d1(
...
async function main() {
  await createAdmin();
  seedDemoRows();
  console.log(`\n✅ Seed complete. Sign in at ${BASE_URL}/login`);
}
